...
public:
  ...
  uint8_t attach(int pin,unsigned int min,unsigned int max); // as above but also sets min and max values for writes. 
  void detach();
  void write(unsigned int value);             // if value is < 200 its treated as an angle, otherwise as pulse width in microseconds 
  void writeMicroseconds(unsigned long value); // Write pulse width in microseconds 
